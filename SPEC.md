...
Broker assumed: standard MQTT 3.1.1, no TLS/auth requirements defined yet (add here when
they exist). All payloads are JSON, UTF-8, no trailing newline.
...
| `request_id` | string (UUID v4) | — | Generated by the Pi. Echoed back in `garden/pump/status` so the two messages can be correlated. |
...
|---|---|---|---|
| `device_id` | string | — | |
| `request_id` | string (UUID v4) | — | Copied verbatim from the triggering `garden/pump/command` message. |
