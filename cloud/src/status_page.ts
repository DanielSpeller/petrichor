export interface StatusRow { device_id: string; wifi_rssi_dbm: number; battery_voltage_v: number; uptime_sec: number; last_seen: number }
...
export function renderPage(status: StatusRow | null, readings: ReadingRow[], events: EventRow[]): string {
  ...
  const content = status ? `<section class="stats"><article><strong>${latest}%</strong><span>moisture</span></article><article><strong>${status.battery_voltage_v.toFixed(2)}V</strong><span>battery</span></article><article><strong>${status.wifi_rssi_dbm} dBm</strong><span>signal</span></article></section><p class="seen">Last seen ${formatTimestamp(status.last_seen)}</p><svg viewBox="0 0 600 120" role="img" aria-label="Moisture history"><polyline points="${buildSparklinePoints(readings)}" fill="none" stroke="currentColor" stroke-width="3" vector-effect="non-scaling-stroke"/></svg><h2>Recent watering</h2><table><thead><tr><th>Time</th><th>Trigger</th><th>Result</th><th>Duration</th><th>Before</th></tr></thead><tbody>${rows || '<tr><td colspan="5">no watering events yet</td></tr>'}</tbody></table>` : '<p>no data yet</p>';
